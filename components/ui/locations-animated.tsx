...
export default function LocationsAnimated() {
  return (
    <motion.div
      className="w-full max-w-[95%] xl:max-w-[1400px] mx-auto px-4 sm:px-6"
    ...
    >
      ...
      >
        {/* XL and above: map + details layout */}
        <div className="hidden xl:grid grid-cols-1 gap-8 items-stretch">
          ...
          >
            <Card className="relative rounded-xl border border-gray-100 overflow-hidden shadow-lg min-h-[360px] lg:min-h-[500px]">
              <Image
                src="/floridaeverglades2.png"
                alt="Map of Consumer Law Florida office locations"
                fill
                draggable={false}
              ...
              <div className="absolute inset-0 pointer-events-none">
                {locationPins.map((location, index) => (
                  <motion.div
                    key={location.id}
                    className={`absolute ${location.position} flex flex-col items-center gap-3 pointer-events-auto ${
                      location.pinOrder === "before" ? "flex-col" : "flex-col-reverse"
                    }`}
        ...
        {/* Below XL: cards grid — loc names use <p> (not <h3>) to avoid duplicate heading entries;
            the XL text-detail grid above owns the canonical H3s for each office. */}
        <div className="block xl:hidden">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {officeLocations.map((loc, idx) => (
              <Card
                key={loc.name}
                className={`rounded-2xl bg-gray-50 border border-gray-100 shadow-md p-6 ${
                  idx === officeLocations.length - 1 ? "md:col-span-2 lg:col-span-1" : ""
                }`}
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <p className="text-xl font-semibold text-gray-900">{loc.name}</p>
                </div>
                <div className="space-y-4 text-gray-700">
                  <div>
                    <div className="text-sm font-medium text-gray-500 mb-1 inline-flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-sm font-medium text-gray-500 mb-1 inline-flex items-center gap-2">
                      <Clock className="w-4 h-4" /> Working /h
                    </div>
